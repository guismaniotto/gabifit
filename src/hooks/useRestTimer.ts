...
export function useRestTimer() {
  ...
  const addTime = useCallback((seconds: number) => {
    ...
    setTotalSeconds((prev) => Math.max(prev + seconds, remaining + seconds));
  }, [remaining]);
